...
///
/// Submitting without changing the selection is harmless and, since
/// 2026-09-08, is also FREE: `change_model` returns early when the
/// requested sprite is the one already equipped, before the charge path.
/// This comment used to say the server "still charges for it if not
/// free", which stopped being true when that guard landed. The picker
/// still does not detect a no-op pick itself - the server does, which is
...
/// the selection.
const HIDEOUT_WARRIOR_ALL_TIP: &str = "Runs the whole Hideout Warrior chain \u{2014} Transmute \u{2192} Augment \u{2192} Regal \u{2192} Exalt \u{2192} Krangle \u{2014} over EVERY eligible item in your bag at once, for dust. Costs exactly what pressing Hideout Warrior on each of those items individually would have cost: no bulk discount, no bulk surcharge. The price on the button is the real total and is checked before anything is crafted \u{2014} if you cannot afford it, nothing happens and nothing is spent. Ignores the item pickers above: this is a whole-bag action. EQUIPPED GEAR IS NEVER TOUCHED, deliberately \u{2014} Krangle is permanent, so worn items can only be Krangled one at a time from the button above. Items already Krangled or ticked \u{1F512} Keep are skipped, not refused. This is Divinity paid in dust instead of a Unique Shard.";
