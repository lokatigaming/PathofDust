...
async fn memories_routes_drive_a_disposable_game_instance_end_to_end() {
    ...
    // isn't counted, and HALVED because `templates/base.html` currently
    // emits `{{ body }}` a second time inside a JS comment (line 685),
    // duplicating every page's whole body - a real pre-existing bug,
    // reported separately and deliberately NOT fixed on this branch.
    // Asserting the observed count rather than the intended one would
    // silently bake the bug in, so this divides instead.
    assert_eq!(html.matches("class=\"memory-slot ").count(), 3 * 2, "all three empty slots must render (x2 while the base-template body duplication stands)");
    assert_eq!(html.matches("Save Current Build").count(), 3 * 2, "all three empty slots must offer a save");
    ...
    assert_eq!(html.matches("Save Current Build").count(), 2 * 2, "one slot is filled now, so only two offer a fresh save (x2, see the note above)");
