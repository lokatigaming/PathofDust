...
SESSION BOUNDARIES (ruling 2026-09-21). **The one-item-per-session rule governs deploys, not merges.** Docs-only merges, whose gate the tree predicts will refuse byte-identical, are batched back to back with one full suite at the end; stop between items only for a deploy, a surprise, or a genuine decision. Items that are ready together may ship as one release. **A session that stops for any reason — including deciding there is nothing to do — writes a report to `C:\dust-work\reports\` saying so before it ends.** **Whether a change moves the binary is measured, not reasoned:** build the merge (on the box, from a scratch worktree) and hash it. A diff that looks cosmetic can still move it, because comment and whitespace edits shift line numbers that are compiled into panic locations and `tracing` metadata (item 17, 2026-09-21).
