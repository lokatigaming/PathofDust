...
3. ~~**Leech's cap window resets instead of sliding.**~~ **FIXED** by the primary session — `drain_leech_window` now implements a genuine leaky-bucket rolling window instead of a reset-based one. Peak leech burst is lower for Slayer builds; steady-state leech is unchanged.
