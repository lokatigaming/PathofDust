...
1. **Echoing Power (Mage) tooltip disagrees with its own data.** `src/passive_tree.rs:1206` description text says "increased from 50% per rank (up to 80% at 3/3)," implying +10%/rank. The stored effect is `Special { at_rank_1: 0.15, per_additional_rank: 0.15 }`, consumed at `src/adventure/combat.rs:7803` — at rank 3 that's 50%+45% = **95%**, not 80%. **Wiki treatment: this node's writeup stays vague (no exact numbers) until the fix lands — publishing either number risks being wrong.**
2. **Intervene's per-character cap doesn't actually hold.** Doc at `src/adventure/character.rs:2770-2781` claims a hard 50%-per-character ceiling. The implementation (`character.rs:2782-2787`) caps the gear source and tree source at 50% each independently, then combines via `combine_reduction_sources` — two sources both at 50% yield 75% combined, not 50%. Same bug shape as two other combine-source bugs in this file that were already found and fixed (evasion, DR) — those got a post-combine hard cap; Intervene never did.
